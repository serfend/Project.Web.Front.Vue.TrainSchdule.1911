...
<div>
  <el-tabs v-model="current_tab" type="border-card" class="tab-container">
    <el-tab-pane :disabled="waitToAuthRegisterUsersLoadId == ''" name="reg" label="用户信息">
      <el-form v-loading="submitLoading" label-position="right" label-width="120px">
        <el-collapse v-model="nowStep" accordion>
          <el-collapse-item v-for="opt in stepOptions.filter((i) => !i.removed)" :key="opt.index" :name="opt.index">
            <template slot="title">
              {{ opt.name }}
              <i :class="opt.icon" />
            </template>
      ...
      <el-button v-if="isToRegister" type="success" :loading="submitLoading" @click="submitRegister(true)">提交注册</el-button>
      <el-button v-if="isToRegister" type="primary" :loading="submitLoading" @click="switchFormType">切换到审核认证</el-button>
      <el-button v-if="!isToRegister" type="warning" :loading="submitLoading" @click="submitRegister(false)">修改信息</el-button>
      <span v-show="
    ...
    <el-tab-pane :disabled="isToRegister" name="lst" label="用户列表">
        ...
        <div slot="header">
          <el-card>
            ...
            <div style="margin-top: 0.5rem">
              <el-button type="primary" :loading="submitLoading" @click="requireLoadWaitToAUthRegisterUsers">刷新待认证人员</el-button>
              <el-button v-if="!isToRegister" type="primary" :loading="submitLoading" @click="switchFormType">切换到普通注册</el-button>
            </div>
          </el-card>
        </div>

        <el-table v-loading="tableLoading" :data="waitToAuthRegisterUsers" highlight-current-row style="width: 100%" @current-change="handleCurrentChange">
          ...
          <el-table-column prop="vacation.description" label="休假详情描述">
            <template slot-scope="scope">
              <span slot="reference">{{
                  scope.row.vacation.description
                }}</span>
            </template>
          </el-table-column>
      ...
      <Pagination v-if="!isToRegister" :pagesetting.sync="MembersQuery" :total-count="MembersQueryTotalCount" />
    </el-tab-pane>
  </el-tabs>
  <el-dialog :visible="init_page_over && not_login_show">
    <SvgIcon icon-class="Message_error" style-normal="width:6rem;height:6rem;margin: auto;color:rgb(243, 126, 125)" class-name="item-put-center" />
    <div class="item-put-center" style="margin: 3rem 0 2rem 0; font-size: 2rem">
      进入注册页面失败
    </div>
    <div class="item-put-center" style="margin: 3rem 0 2rem 0; font-size: 1rem">
      为保证您的账号安全
    </div>
