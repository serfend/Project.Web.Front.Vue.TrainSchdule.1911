...
const calendar = {
   ...
   * @param lunar Year
   * @return Number
   * @eg:const count = calendar.lYearDays(1987) ;//count=387
   */
   ...
   * @param lunar Year
   * @return Number (0-12)
   * @eg:const leapMonth = calendar.leapMonth(1987) ;//leapMonth=6
   */
   ...
   * @eg:const leapMonthDay = calendar.leapDays(1987) ;//leapMonthDay=29
   */
  leapDays: function (y) {
   ...
   * @eg:const MonthDay = calendar.monthDays(1987,9) ;//MonthDay=29
   */
   ...
   * @eg:const solarMonthDay = calendar.leapDays(1987) ;//solarMonthDay=30
   */
   ...
   * @return day Number
   * @eg:const _24 = calendar.getTerm(1987,3) ;//_24=4;意即1987年2月4日立春
   */
   ...
   * @param lunar month
   * @return Cn string
   * @eg:const cnMonth = calendar.toChinaMonth(12) ;//cnMonth='腊月'
   */
  ...
  getSunday(y, m, n) {
    ...
    d2.setDate(0)
    const maxDay = d2.getDate() // 传入月份的最后一天

    const week = d.getDay()
    let first = 1
    if (week > 0) first = d.getDate() + (7 - week)
    const result = (n - 1) * 7 + first
   ...
   * @param lunar day
   * @return Cn string
   * @eg:const cnDay = calendar.toChinaDay(21) ;//cnMonth='廿一'
   */
   ...
   * @param y year
   * @return Cn string
   * @eg:const animal = calendar.getAnimal(1987) ;//animal='兔'
   */
  ...
  solar2lunar: function (y, m, d) { // 参数区间1900.1.31~2100.12.31
    ...
    // 未传参  获得当天
    const objDate = y ? new Date(y, parseInt(m) - 1, d) : new Date()
    let i; let leap = 0
    let temp = 0
    // 修正ymd参数
   ...
   */
  getSolar(date) {
    if (!date || date.indexOf('L') === -1) return null
    const isLeapMonth = date.indexOf('R') > -1
    const D = date.replace(/L/, '').replace(/R/, '').split('-')
    const cY = parseInt(D[0])
    const cM = parseInt(D[1])
    const cD = parseInt(D[2])
